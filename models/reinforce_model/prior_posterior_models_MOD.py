...
from models.reinforce_model.dataset_MOD import EFFECTS
...
class PriorBoWModel(nn.Module):
    ...
    def get_prob_z_given_H(self, persona, history, effects=None):
        ...
        if self.uniform_prior:
            ...
            if self.args.greedy_profile:
                dists=torch.norm(history_encodings - persona_encodings, 2, dim=-1)
            else:
...
class InferenceRobertaModel(nn.Module):
    ...
    def get_prob_z_given_H_and_x(self, gt_response_ids, persona, history, effects=None,prior_model=None):
        '''
        persona: B x P x T
        H: B x T
        We take the pooled output from Roberta; which uses same tokenization as GPT2
        ...
        if self.uniform_prior:
            ...
            if self.use_history:
                persona_hist=persona.clone()
            batch_size, num_personas, num_tokens = persona.shape
            persona = persona.reshape(-1, num_tokens)
            # print("persona.shape:", persona.shape)
            persona_encodings = self.roberta_model(persona)[1][-1][:, 0, :].reshape(batch_size, num_personas, -1)
            if self.use_history:
                z_given_h_dist = prior_model.get_prob_z_given_H(persona_hist, history, effects)  # B x P
                if not self.mixed:
                    return z_given_h_dist.reshape(1)
                #raise NotImplementedError
            if self.args.greedy_profile:
                dists = torch.norm(gt_response_encodings - persona_encodings, 2, dim=-1)
                if not self.mixed:
                    z = torch.argmin(dists)
                    return torch.tensor([z],device=self.args.device)
                else:
                    pre_sel_hist=torch.topk(z_given_h_dist,self.args.max_personas,largest=False)
                    pre_sel_gt=torch.index_select(dists,-1,pre_sel_hist.indices)
                    sel_gt_index = torch.argmin(pre_sel_gt).item()
                    return pre_sel_hist.indices[sel_gt_index].to(self.args.device)
