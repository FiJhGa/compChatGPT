...
from models.reinforce_model.utils_MOD import get_dataset, make_logdir
from models.reinforce_model.data import PADDED_INPUTS, ATTR_TO_SPECIAL_TOKEN
# from models.reinforce_model.dataset import PersonaChatDataset
...
from models.reinforce_model.interact_MOD import sample_sequence
...
parser.add_argument("--compac", type=bool, default=False, help="Nucleus filtering (top-p) before sampling (<=0.0: no filtering)")
parser.add_argument("--greedy_profile", type=bool, default=False, help="Nucleus filtering (top-p) before sampling (<=0.0: no filtering)")
parser.add_argument("--ppl", type=bool, default=False, help="Calculate ppl")
...
# training_args.greedy_profile=False
# training_args.use_history=False
...
if args.compac == True:
    ...
    from models.reinforce_model.dataset_MOD import PersonaChatDataset
    split='test'    
val_dataset = PersonaChatDataset(args, tokenizer, split=split, history_folded=True)
...
for i, item in tqdm(enumerate(val_dataset), total=len(val_dataset)):
    ...
    with torch.no_grad():
        gen_dict = {}
        #input_ids, token_type_ids, mc_token_ids, lm_labels, mc_labels, persona, history, effects, gt_response_ids = item
        input_ids=item["input_ids"]
        lm_labels=item["lm_labels"]
        gt_response_ids=item["gt_response_ids"]
        ...
        if not args.ppl:
            ...
            for h in history:
                if isinstance(h, list):
                    ...
                    history_texts.append(tokenizer.decode(history, skip_special_tokens=True))
                    history=[history]
                    break
        ...
        #         history=[history]
        #         break        
        # history_texts = tokenizer.decode(" ".join(history), skip_special_tokens=True)
        # if i == 0:
        ...
        #ground_truth = [t for t in lm_labels[0] if t!= -100]
        #ground_truth = [t for t in lm_labels[0][0] if t!= -100]

        if args.ppl:
            out_ids, z, loss= sample_sequence(persona, history, tokenizer, model, args, current_output=gt_response_ids[0], persona_choice=None)
        else:
            out_ids, z, loss= sample_sequence(persona, history, tokenizer, model, args, current_output=None, persona_choice=None)
            out_text = tokenizer.decode(out_ids, skip_special_tokens=True)
            retrieved_persona = tokenizer.decode(persona[z][1:], skip_special_tokens=True)            
            ground_truth_text = tokenizer.decode(gt_response_ids[0], skip_special_tokens=True)

        #out_ids, z = sample_sequence(persona, history, effects, tokenizer, model, args, current_output=None, persona_choice=None)
        ...
        if not args.ppl:
            generated["persona"].append(retrieved_persona)
            generated["history"] = history_texts
            generated["predictions"].append(out_text)
            generated["references"].append(ground_truth_text)
