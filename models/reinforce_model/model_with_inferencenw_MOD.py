...
from models.reinforce_model.prior_posterior_models_MOD import PriorRobertaModel, InferenceRobertaModel
...
class LatentVariableInferenceModel(nn.Module):
    ...
    def forward(
        ...
        if not generate:
            ...
            z_given_h_and_x = sampler_model.get_prob_z_given_H_and_x(gt_response_ids, persona, history, effects,prior_model=self.prior_model)  # B x P
            ...
            if self.training_type == TRAINING_TYPE_MARGINALIZE:
                ...
                if self.args.greedy_profile:
                    z_iterator = [z_given_h_and_x]# int
                    logprob_action = 1
                else:
            ...
            for i in z_iterator:
                ...
                if self.training_type == TRAINING_TYPE_MARGINALIZE:
                    ...
                    #lm_logits, mc_logits, *_ = self.gpt2_model(
                    out_gpt2 = self.gpt2_model(
                        input_ids,
                        token_type_ids=token_type_ids,
                        mc_token_ids=mc_token_ids,
                    )
                lm_logits=out_gpt2.logits
                mc_logits=out_gpt2.mc_logits
            ...
            if self.training_type == TRAINING_TYPE_MARGINALIZE:
                ...
                if not self.args.greedy_profile:
                    loss_prior = - logprob_action * rewards  # B
                    loss_prior = loss_prior.mean()  # B
                else:
                    loss_prior = torch.tensor([0],device=self.args.device)           
            ...
            if not self.args.greedy_profile:
                ...
                return lm_logits, mc_logits, total_loss_lm, loss_mc, loss_prior, loss_lm, num_labels, track_rewards
