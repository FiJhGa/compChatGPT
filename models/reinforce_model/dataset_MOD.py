...
from models.reinforce_model.utils_MOD import get_dataset, make_logdir, preprocess
...
def build_input_from_segments(persona, history, reply, tokenizer, lm_labels=False, with_eos=True, loss_calculation=False,gt_response_labels=None):
    ...
    if lm_labels:
        ...
        instance["gt_response_ids"] = sequence[-1][1:]
...
class PersonaChatDataset(Dataset):
        ...
        for d_i, dialog in tqdm(enumerate(personachat_split), total=len(personachat_split)):
            ...
            if not args.no_comet_persona:
                ...
                for effect_name in comet_annotations:
                    ...
                    # if d_i == 0 and j_s == 0:
                    #     print('For a sent: \n{}'.format(sent['comet']))
                    #for j_s, effect in enumerate(comet_annotations[effect_name]):
                        # if effect_name in EFFECTS:
                    # logging
                    if d_i == 0:
                        print('Getting data for effect {}'.format(effect_name))
                        if args.num_beams != -1:
                            print('Getting {} beams'.format(len(comet_annotations[effect_name][:args.num_beams])))
                        else:
                            print('Getting {} beams'.format(len(comet_annotations[effect_name])))

                    if args.num_beams != -1:
                        sent_beams += comet_annotations[effect_name][:args.num_beams]
                        effects += [EFFECTS[effect_name]] * args.num_beams
                    else:
                        sent_beams += comet_annotations[effect_name]
                        effects += [EFFECTS[effect_name]] * len(comet_annotations[effect_name])                       
                if d_i == 0:
    ...
    def collate_dialog(self, batch):
        ...
        for name in batch[0].keys():
            ...
            elif name == "gt_response_ids":
                padded_batch[name] = torch.LongTensor([sample[name] for sample in batch])\
                    .view((max_num_persona, -1,  self.num_candidates))                            
