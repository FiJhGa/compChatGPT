...
from models.reinforce_model.dataset_MOD import PersonaChatDataset, MAX_NUM_PERSONA, MAX_NUM_COMET_PERSONA
...
def get_args():
    ...
    parser.add_argument("--use_history", type=bool, default=False, help="Use history of dialogs")
    parser.add_argument("--mixed", type=bool, default=False, help="Use mixed persona selection")
    ...
    def inference(engine, batch):
        ...
        with torch.no_grad():
            ...
            lm_logits, mc_logits, *_ = model(
                ...
                gt_response_ids=batch["gt_response_ids"],
            )
...
def create_trainer_and_checkpoint_handler(args, model, optimizer, train_loader, val_loader, evaluator, log_dir):
    def update(engine, batch):        
        ...
        if not args.greedy_profile:
            ...
            _, _, lm_loss, mc_loss, loss_prior, conditional_lm_loss, num_labels, track_rewards= model(
            input_ids=batch["input_ids"],
            token_type_ids=batch["token_type_ids"],
            mc_token_ids=batch["mc_token_ids"],
            lm_labels=batch["lm_labels"],
    ...
    # Define your early stopping handler
    def score_function(engine):
        val_loss = engine.state.metrics['loss']
        return -val_loss
    
    early_stopping = EarlyStopping(patience=2, score_function=score_function, trainer=trainer)
